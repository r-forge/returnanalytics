...
integrateSubIntervals <- function(x, cdf) {
  ...
  p <- (cdf(bounds$xUB) - cdf(bounds$xLB)) / (bounds$xUB - bounds$xLB)
...
hermitePolynomial <- function(n) {
  ...
  if(n >= 2) {
    for (k in 2:n) {
      p[k + 1, 1:(k + 1)] <- 2 *  cbind(t(p[k, 1:k]), 0)  - 2 * (k - 1) *
                            cbind(0, 0, t(p[k - 1, 1:k - 1]))
    }
  }

  return(p)
}
#' @title Generates grid representation of a distribution
#'
#' @description Generates grid representation of a distribution according to the
#' method suggested by Meucci and inspired from Gauss-Hermite quadratures.
#'
#' @details Grid representation by this method is an alternative to representing
#' distribution of the market, such as importance sampling or stratified
#' sampling. However, these techniques focus on sub-domains of the distribution,
#' and thus, in order to apply such methods, we must forego the full flexibility
#' on the specification of the views. A different approach,  which preserves the
#' generality of the views specification, consists in selecting the scenarios
#' x_j deterministically as a  grid and then associate with each of them the
#' suitable probability p_j (integrated over I_j). The the generic interval _j
#' contains the j-th point of the grid. Once the grid is defined, the entropy
#' optimization can be applied to replace p_j with the new posterior
#' probabilities to reflect the views. We generate the grid here using the
#' Gauss-Hermite quadrature method.
#'
#' @param J   a numeric containing the number of points on the grid
#'
#' @return x  a matrix containing the zeroes of Hermite polynomials as a
...
#' Generates bandwidth of a Gaussian Kernel Density Estimator based on
#' Silverman's rule of thumb
#'
#' @param xi  a vector containing the data set for which the bandwidth has
#'            to be calculated
#'
...
#'
#' @param x   a vector containing the numeric values for which cumulative
#'            distribution function has to be evaluated
#' @param xi  a vector containing the data set
#' @param bw  a numeric value containing Silverman bandwidth of the
#'            given data set
#' @param wi  a vector containing weights
#'
#' @return p  a numeric containing the cumulative probability distribution
...
#'
#' @param x   a vector containing the numeric values for which probability
#'            distribution function has to be evaluated
#' @param xi  a vector containing the data set
#' @param bw  a numeric value containing Silverman bandwidth of the given data
#'            set
#' @param wi  a vector containing weights
#'
#' @return p  a vector containing the probability distribution function value of
#'            length equal to x
#'
#' @author Manan Shah \email{mkshah@@tepper.cmu.edu}
...
#' Evaluates inverse probability distribution function for the input probability
#' in order to get the data point
#'
#' @param p   a vector containing the probabilities corresponding to which the
#'            data points have to be evaluated
#' @param xi  a vector containing the data set
#' @param bw  a numeric value containing Silverman bandwidth of the given data
#'            set
#' @param wi  a vector containing weights
#'
#' @return x  a vector containing the evaluated numeric values of length equal
#'            to p
#'
#' @author Manan Shah \email{mkshah@@tepper.cmu.edu}
...
kernelinv <- function(p, xi, bw, wi) {
  ...
  if (length(p) < 10) {
    ...
    x <- rep(0, length(p))
    for (i in 1:length(p)) {
      x[i] <- uniroot(function(x) private_fun(x, xi, bw, wi, p[1]),
                     c(-100, 100))$root
    }
  } else {
  # case with many points by interpolation, find x_min and x_max
  x_min <- uniroot(function(x) private_fun(x, xi, bw, wi, sortp[1]),
                  c(-100, 100))$root
  x_max <- uniroot(function(x) private_fun(x, xi, bw, wi, sortp[nrow(sortp)]),
                  c(-100, 100))$root
...
#' Evaluates the difference between calculated cumulative distribution function
#' for a data point and the true value
#'
#' @param x   a vector containing the data points for which cdf has to be
#'            evaluated
#' @param xi  a vector containing the data set
#' @param bw  a numeric value containing Silverman bandwidth of the data set
#' @param wi  a vector containing weights
#' @param p   a vector containing the true probabilities
#'
#' @return f  a vector containing the difference between x and p which is of
