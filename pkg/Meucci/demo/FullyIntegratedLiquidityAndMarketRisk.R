#' This script computes the liquidity-risk and funding-risk adjusted P&L 
#' distribution, as described in A. Meucci, "A Fully Integrated Liquidity and
...
# select only some stock in portfolio and equally allocate capital as fraction
# of daily dollar volume
Selectstock <- 1:10 
...
Daily_LogRets <- log(fILMR$Daily_Prices[-nrow(fILMR$Daily_Prices), ] /
                     fILMR$Daily_Prices[-1, ])
J <- dim(Daily_LogRets)[1]
N <- dim(Daily_LogRets)[2]

# volumes in shares
#Daily_Volumes = Daily_Volumes_Shares[, Selectstock]
Volumes_0 <- matrix(fILMR$Daily_Volumes_Shares[
                    nrow(fILMR$Daily_Volumes_Shares), ])

Volumes_t <- matrix(apply( fILMR$Daily_Volumes_Shares[
                     -(1:(nrow(fILMR$Daily_Volumes_Shares)-250)), ], 2, mean))
...
NonLinear <- -(10^5) * Prices_0 * matrix(apply(Daily_LogRets, 2, sd)) * 
                                         ((abs(Dh)  / Volumes_t) ^ 1.5)        
...
nu   <- 100
f_Pi <- function(x) {
  t(Probs / s_j) %*% dt((x-m_j) / s_j, nu)
}

F_Pi <- function(x) {
  t(Probs) %*% pt((x-m_j) / s_j, nu)
}
...
# plot histogram
hh <- plot(hist$mids, hist$counts / (J * D), type = "h", xlab ="", ylab = "")
hh1 <- lines(x_,f_Pi_plot, col = "red")

legend("topright", 1.9, c("pure market P&L","market + liquidity P&L"),
       col = c("black", "red"), lty = 1, bg = "gray90")
