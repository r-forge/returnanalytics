#' This script considers a bivariate lognormal market and display the
#'  correlation and the condition number of the covariance matrix, as described
...
for (n in 1 : nrhos) {
    rho <- rhos[n]
    Sigma <- rbind(c(s[1] ^ 2, rho * s[1] * s[2]), c(rho * s[1] * s[2],
    		 s[2] ^ 2))
