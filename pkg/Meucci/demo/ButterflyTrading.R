...
################################################################################
# Load panel X of joint factors realizations and vector p of respective 
# probabilities. In real life, these are provided by the estimation process
################################################################################
...
Options$FrontierSpan <- cbind(0.1, 0.8)  # range of normalized exp.vals. spanned 
...
optimalPortfolios <- LongShortMeanCVaRFrontier(PnL, 
											  as.matrix(factorsDistribution$p),
											  butterfliesAnalytics, Options)

# View(optimalPortfolios) 
# Note that composition is measured in dollars. Here we are short GOOG_vol_91 
# and long GOOG_vol_182

PlotFrontier(optimalPortfolios$Exp, optimalPortfolios$CVaR, 
		     optimalPortfolios$Composition)
...
# View 1 (inequality view): bearish on on 2m-6pm implied volaility spread for
# Google. Note that the mean-CVaR efficient frontier is long based on the 
# reference model. After processing the view, the G6m-G2m spread is now short in
# the mean-CVaR long-short efficient frontier

p_2 <- ViewRealizedVol(factorsDistribution$X, factorsDistribution$p);

# view 2 (relative inequality view on median): bullish on realized volatility of
# MSFT (i.e. absolute log-change in the underlying). This is the variable such
# that, if larger than a threshold, a long position in the butterfly turns into
# a profit (e.g. Rachev 2003)we issue a relative statement on the media 
...
# view 3 (equality view - expectations and binding constraints): slope of the
# yield curve will increase by 5 bp
 
# assign confidence to the views and pool opinions
# sum of weights should be equal 1
# .35 is the weight on the reference model
# .2 is the confidence on View 1; .25 is the confidence on View 2; .2 is the 
# confidence on View 3

c <- cbind(0.35, 0.2, 0.25, 0.2)
# compute the uncertainty weighted posterior probabilities
p_ <- cbind(factorsDistribution$p, p_1, p_2, p_3) %*% t(c) 
...
################################################################################
#[Exp_,SDev_,CVaR_,w_]
optimalPortfoliosPosterior <- LongShortMeanCVaRFrontier(PnL, p_, 
													   butterfliesAnalytics, 
													   Options)
#PlotEfficientFrontier(Exp_,CVaR_,w_)

View(optimalPortfoliosPosterior) # Note that composition is measured in dollars. 
# Now we are long GOOG_vol_91, and short Goog_vol_182

# plot efficient frontier (Exp_, CVaR_, w_)
PlotFrontier(optimalPortfoliosPosterior$Exp, optimalPortfoliosPosterior$SDev,
			 optimalPortfoliosPosterior$Composition)
################################################################################
# tests
################################################################################
