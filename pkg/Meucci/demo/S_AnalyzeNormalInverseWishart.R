...
#' The ensuing normal-inverse-Wishart posterior is computed and analyzed
...
#' \url{http://symmys.com/node/170}, "E 282 - Bayesian: normal-inverse-Wishart
#' posterior".
#'
...
# plot results
PlotMarginalsNormalInverseWishart(RNIWPrior$Mu, RNIWPrior$InvSigma, Mu_0, T_0,
								  Sigma_0, Nu_0, "prior")
...
Nu_1 <- Nu_0 + T
Sigma_1 <- (Nu_0 %*% Sigma_0 + T %*% Sigma_Hat + (T %*% T_0 / (T + T_0)) %*%
		   (Mu_0 - Mu_Hat) %*% t(Mu_0 - Mu_Hat)) / Nu_1

# generate simulations
RNIWPost <- RandNormalInverseWishart(Mu_1, T_1, Sigma_1, Nu_1, nSim)

# plot results
PlotMarginalsNormalInverseWishart(RNIWPost$Mu, RNIWPost$InvSigma, Mu_1, T_1,
								  Sigma_1, Nu_1, "posterior")
