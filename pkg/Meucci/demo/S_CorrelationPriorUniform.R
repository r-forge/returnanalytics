...
#' that the marginal distribution of each correlation is peaked around zero, as
...
#' \url{http://symmys.com/node/170}, "E 281 - Bayesian: prior on correlation".
#'
...
for(k in 1 : K) {
  dev.new()
  hist(CorrsAsEntries[k, ]$Values, Nbins, xlab = "", ylab = "",
       main = bquote(paste("histogram of ", theta, 
       .(CorrsAsEntries[k, ]$Names))))
}
...
for (k in 1 : (K-1)) {
  for (j in (k + 1) : K) {
    ...
    plot(CorrsAsEntries[k, ]$Values,CorrsAsEntries[j, ]$Values,
         xlab = "", ylab = "", main = paste(CorrsAsEntries[k]$Names, ' - ',
         CorrsAsEntries[j]$Names))
  }
}
