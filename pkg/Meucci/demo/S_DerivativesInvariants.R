#' This script performs the quest for invariance in the derivatives market, as
...
#' \url{http://symmys.com/node/170}, "E 142 - Derivatives market: quest for
#' invariance".
#'
...
################################################################################
### Load implied vol for options on SPX for different time to maturity and
### moneyness
...
# select the implied vol time series for a specific time to maturity and
# moneyness
maturityIndex  <- 1   # 1..6
moneynessIndex <- 4   # 1..7

################################################################################
### Quest for invariance for changes in implied vol and changes in log implied
### vol
...
Y <- diff(log(derivatives$impVol[eachFiveRowsSeq, maturityIndex,
		  moneynessIndex]))
...
Z <- matrix(log(derivatives$impVol[eachFiveRowsSeq , , ]), Dim[1], Dim[2] *
		    Dim[3])
