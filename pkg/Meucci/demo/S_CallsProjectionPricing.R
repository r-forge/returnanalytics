...
# current time to maturity of call options in days
Time2Mats <- c(100, 150, 200, 250, 300)
# strikes of call options, same dimension as Time2Mat
Strikes   <- c(850, 880, 910, 940, 970)
...
################################################################################
### Linearly interpolate the vol surface at the current time to obtain implied
### vol for the given calls today, and price the calls
...
impVol_T    <- t(InterExtrapolate(volSurf_T,t(rbind(time2Mat_T, moneyness_T)),
               list(implVol$days2Maturity, implVol$moneyness)))
callPrice_T <- BlackScholesCallPrice(spot_T, Strikes, r_free, impVol_T,
               Time2Mats / 252)$c
...
for (j in 1 : J) {
    volSurf       <- volSurf_T * exp(matrix(X_[j, -1],
                     length(implVol$days2Maturity),length(implVol$moneyness)))
    time2Mat_     <- Time2Mats - tau
    moneyness_    <- Strikes / spot_[j]
    impVol_[j, ] <- t(InterExtrapolate(volSurf, t(rbind(time2Mat_T,
                    moneyness_T)), list(implVol$days2Maturity,
                    implVol$moneyness)))
}
...
for (i in 1 : numCalls) {
    callPrice_[, i] <- BlackScholesCallPrice(spot_, Strikes[i], r_free,
                       impVol_[, i], time2Mat_[i] / 252)$c
}
